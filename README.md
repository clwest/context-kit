...
> shipping production code with an AI pair. Concrete example of a rule
> that has an incident behind it: `init` refuses to overwrite an existing
> project unless `--force` is passed, because an early session lost hand-
> edited docs to a well-meaning re-run. See `docs/TRUST_CALIBRATION.md`
> for the append-only log of moments where AI was confidently wrong or
> right despite pushback.
...
Load-bearing terms used throughout the docs:

- **Narrative anchor** — the hand-written "what is this project?" doc,
  e.g. `docs/MY_APP_WHAT_IT_IS.md`. Owned by humans. Source of truth
  for meaning.
- **Runtime anchor** — the auto-regenerated "what does the code actually
  contain right now?" doc, e.g. `docs/MY_APP_INVENTORY.md`. Owned by
  the `inventory` command. Source of truth for facts.
- **Drift verifier** — the `verify` command. Compares claims in docs
  against evidence in code/config; reports conflicts.
- **Session handoff** — a Markdown file in `docs/handoffs/` capturing
  one session's decisions, calibration moments, and open threads.
  Handed off to the next session (human or AI).
- **Managed block** — a marker-delimited region inside a doc that is
  regenerated by a command; text outside the markers is preserved as
  human-authored.
...
- Scaffolding new projects with a memory/anchor layer.
- Retrofitting the pattern onto existing repos (`adopt`).
- Detecting doc-vs-runtime drift (`verify`, `inventory --check`).
- Session-continuity artifacts (`orient`, `handoff`, session-start docs).
- Read-only repo inspection (`inspect`, `capabilities`, `hotpath`).
- Bundled Claude Code skill for automatic orientation.

**Out of scope:**

- Replacing your test runner, linter, formatter, or CI.
- Running or supervising AI agents. context-kit prints prompts;
  it does not call any model API.
- Cloud sync, hosted state, telemetry, or authentication.
- Framework-specific integrations beyond what's opt-in under
  `starter/scaffold/`.
- Enforcing a particular language, framework, or workflow.
...
`context-kit init "My App"` produces roughly:

```
my-app/
├── 00-START-NEXT-SESSION.md           # read this first each session
├── CLAUDE.md                          # AI collaboration rules
├── AGENTS.md                          # same, aliased for other tools
├── .claude/skills/context-kit/        # auto-loading Claude Code skill
├── docs/
│   ├── MY_APP_WHAT_IT_IS.md           # narrative anchor (hand-written)
│   ├── MY_APP_INVENTORY.md            # runtime anchor (regen'd)
│   ├── TRUST_CALIBRATION.md           # append-only AI-calibration log
│   ├── docs-pattern/                  # the 8 teaching guide docs
│   ├── topics/                        # per-topic reference docs
│   └── handoffs/
│       └── SESSION_001_BOOTSTRAP.md   # first session handoff
└── cli/                               # runtime subcommand copy
```
...
### `inspect` framework scope

The `inspect` command detects any repo's primary language from manifest
files (`pyproject.toml`, `package.json`, `Cargo.toml`, `go.mod`,
`Gemfile`, `composer.json`, `foundry.toml`, etc.), but only two stacks
get **deeper framework-specific probes** in v1:

- **Django** — counts URL patterns, apps, models, management commands,
  and Celery task decorators.
- **Next.js** — counts route files under `app/` and `pages/`.

Other stacks (FastAPI, Flask, React, Vite, Expo, Vue, Svelte, Rust,
Go, Ruby, PHP, Solidity, etc.) are detected as their primary language
via manifest presence, and framework hits are reported when they show
up in file content, but no framework-specific signal counts are produced.
This is intentional and documented at `cli/inspect.py:12` — the goal is
honest under-claiming rather than pretending to understand every stack.
