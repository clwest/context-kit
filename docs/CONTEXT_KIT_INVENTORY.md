...
**0.7.0** is **published to PyPI**, tagged `v0.7.0` on `origin`,
and has a GitHub release page attached to the tag.

**0.8.0 is built on `origin/main` but NOT released.** Fourteen
commits land the v0.8.0 "decision layer" (depth-2 workspace
walk + Adopt Summary card + Stack reality + Project type +
Suggested actions + workspace-aware detection + diagnostic-
signals copy softening + context-aware actions). See
`docs/handoffs/SESSION_010_ADOPT_V0_8_DECISION_LAYER.md` for the
per-ship breakdown. PyPI version, `pyproject.toml` version, and
`CHANGELOG.md` `[Unreleased]` block are all still on / awaiting
0.8.0 release prep.
...
<!-- Last generated: 2026-04-27T05:11:34+00:00 -->
<!-- Schema version: 1 -->
