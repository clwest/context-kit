...
**0.7.0** is **published to PyPI** and tagged `v0.7.0` on `origin`
(annotated, points at `ff5b87c`). Fresh-venv `pip install
contextkit-ai==0.7.0` + `context-kit adopt --help` verified against
the PyPI artifact. GitHub release for `v0.7.0` is the only
...
<!-- Last generated: 2026-04-27T01:30:31+00:00 -->
<!-- Schema version: 1 -->
