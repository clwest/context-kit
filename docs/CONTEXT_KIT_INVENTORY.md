...
| `refactor track` | `cli/refactor.py` + `cli/_refactor/` | Yes | No |
...
- **0.15.0** adds `refactor track`, a read-only progress reporter
...
<!-- Last generated: 2026-05-03T01:09:52+00:00 -->
<!-- Schema version: 1 -->
...
| Item | Count | Notes |
|---|---|---|
| CLI subcommands | 16 | adopt, audit, doctor, exec, fix, hotpath, init, inspect, inventory, orient, recommend-stack, refactor, seed, start, start-codex, translation-init |
| Python modules in `cli/` | 19 | __init__.py, adopt.py, audit.py, bootstrap.py, doctor.py, exec.py, fix.py, hotpath.py, inspect.py, inventory.py, orient.py, placeholders.py, recommend_stack.py, refactor.py, seed.py, server.py, start_codex.py, state.py, translation_init.py |
| Top-level guide docs | 8 | matches `0[1-8]_*.md` |
| `docs/` files (top-level) | 5 | excludes handoffs |
| Session handoffs | 13 | `docs/handoffs/` |
| Templates | 0 | `templates/` |
| Starter files (excl. scaffold) | 0 | `starter/` |
| Scaffold files | 0 | `starter/scaffold/` |
| Test files | 21 | `tests/test_*.py` |
| Tests collected | 818 | from `def test_` parse |
| Skill files | 2 | `skills/**/SKILL.md` |
| Tracked files | 116 | from `git ls-files` |
