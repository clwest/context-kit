...
**0.7.0** through **0.16.0** are published release lines. See
...
- **0.16.0** is the public-portfolio release: folds ~62 post-0.15.0
  commits into a shippable wheel so the wheel matches the README.
  Publishes `chat`, `verify`, `capabilities`, `coverage`, `behavior`,
  `connections`, `handoff`, `translation-init`, `codex`, `start-codex`,
  and `audit-response` (all previously in the code tree but absent from
  the 0.15.0 wheel), the runtime-state block in `orient`, the extended
  doctor drift checks, and the `spokesperson-corpus` /
  `fleet-network` bundled sub-patterns. See `CHANGELOG.md` for the
  full rollup.

`[Unreleased]` is currently empty following the 0.16.0 rollup.
...
<!-- Last generated: 2026-08-12T18:58:21+00:00 -->
<!-- Schema version: 1 -->
...
| Python modules in `cli/` | 28 | __init__.py, adopt.py, audit.py, audit_response.py, behavior.py, bootstrap.py, capabilities.py, chat.py, connections.py, coverage.py, doctor.py, exec.py, fix.py, handoff.py, hotpath.py, inspect.py, inventory.py, ollama.py, orient.py, placeholders.py, recommend_stack.py, refactor.py, seed.py, server.py, start_codex.py, state.py, translation_init.py, verify.py |
| Top-level guide docs | 8 | matches `0[1-8]_*.md` |
| `docs/` files (top-level) | 4 | excludes handoffs |
| Session handoffs | 18 | `docs/handoffs/` |
| Templates | 0 | `templates/` |
| Starter files (excl. scaffold) | 0 | `starter/` |
| Scaffold files | 0 | `starter/scaffold/` |
| Test files | 32 | `tests/test_*.py` |
| Tests collected | 1085 | from `def test_` parse |
| Skill files | 2 | `skills/**/SKILL.md` |
| Tracked files | 160 | from `git ls-files` |
