...
> **Origin:** first landed inside `unified-donkey-betz/docs/docs-pattern/`
> in u-d-b SESSION_1117 (2026-05-21) as a working example, then lifted into
> the context-kit bundle in SESSION_019 (2026-05-21) so every project
> generated by `context-kit init` ships it. The proposed
> `context-kit fleet-net init <fleet-name>` CLI subcommand is not yet
> implemented — see `docs/proposals/fleet-network-subcommand.md` in the
> context-kit repo for the design. Adopters use the `cp -r` workflow until
> the CLI lands. See `RECIPE.md` for the manual steps the subcommand
